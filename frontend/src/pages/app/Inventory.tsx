...
export function Inventory() {
  ...
  const filtered = Object.entries(grouped).reduce((acc, [cat, items]) => {
    const filteredItems = items.filter(r =>
      r.name.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '')
          ...
          <div key={cat}>
            ...
            <div className="flex flex-col gap-2">
              {items.map(resource => (
                <div
                  key={resource.id}
                  className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 gap-4"
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <input
                      type="number"
                      min={0}
                      value={inventory[resource.id] ?? 0}
                      onChange={e => handleQuantityChange(resource.id, Number(e.target.value))}
